...
const activateLightBox = (thisElmt) => {
  ...
  overlay.classList.add('show');   // 오버레이에 show 클래스를 추가하며 활성화
};

// 이미지 요소들을 순회화며 각각을 클릭했을 때 activeLightBox 함수 실행 : 클릭한 대사 용소를 파라미터로 전달
images.forEach((img) => {
...
// 이미지 요소 배열을 순회한다. 첫번째 파라미터는 이미지 요소 배열, 두번째 파라미터는 boolean 값 - true일 경우? 뒤에 : 을 기준으로 앞에 로직이 적용되고, false일 경우? 뒤에 로직이 적용된다.
...
function showItems(thisItem) {
  ...
  if (thisCategory === 'all') { //클릭한 요소의 속성값이 all 일 때
